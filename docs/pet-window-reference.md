...
## 桌宠换行与输入提示（2026-09-12，partial）

桌宠最终消息复用视频通话 turnIngest 的换行分段与文本归一化，逐段排队显示；
连续消息不覆盖未读段落，同一 msg_id 重投不重复播放，贴纸只随首段显示。
主窗口转发过滤群聊、Dream 和非当前角色。聊天输入栏移除快捷键说明，键盘行为保留。
验证：petDialogue 两项纯逻辑回归通过；生产构建另见本次施工结果。
open：真实 Tauri 桌宠分段停留、语音播放时长和多窗口切换尚未实测。
本单没有新增后端/手机设置、存储或协议；手机仍消费原消息，后端管理面无新增开关。
