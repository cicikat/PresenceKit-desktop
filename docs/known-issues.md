...
- **可信设计 Mod 真实窗口 fixture 验收** — `open`。54/55/57 的默认高度链、宿主恢复入口、纯逻辑测试和 presenter 生命周期已通过，代码层“Ribbon 底部控件消失”问题已关闭；仍需在真实窗口内选择 `freeform-capability-fixture`，确认“不挂官方 Status、只消费 `host.presenters.status`”以及 800x600/宽桌面/窄窗口、拖动/resize/Sidebar 变化/原生移动及 20 次切换，并单独验证 release 包的 `resource_dir/design-mods`。
