...
import { dreamGetState, dreamGroupGetState } from '../../../shared/api/dream';
...
// 成功后）才恢复。
export function useDreamState(mode: 'single' | 'group' = 'single', groupId: string | null = null) {
  const [dreamState, setDreamState] = useState<DreamState | null>(null);

  const fetcher = useCallback(async () => {
    const s = mode === 'group' && groupId ? await dreamGroupGetState(groupId) : await dreamGetState();
