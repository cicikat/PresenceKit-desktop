...
export function AgentRuntimeBrowserSettingsPage() {
  ...
  const [form, setForm] = useState({ url: '', operation: 'read_page', idempotencyKey: '' });
  ...
  const submit = async () => {
    ...
    const request: BrowserTaskRequest = { charId, url: form.url.trim(), operation: form.operation, idempotencyKey: form.idempotencyKey.trim() || newIdempotencyKey() };
    ...
    try {
      ...
      pendingRequests.current.set(task.task_id, request); setForm(current => ({ ...current, url: '', idempotencyKey: '' }));
  ...
  const act = async (task: AgentRuntimeTask, action: 'confirm' | 'pause' | 'cancel') => {
    ...
    if (!request) { setActionError(t('settings.agentRuntime.controlUnavailable')); return; }
    setActionError(null);
    try {
  ...
  return <div style={{ display: 'grid', gap: 14 }} data-testid="agent-runtime-browser-settings">
    ...
    {tasks.length > 0 && <div style={{ display: 'grid', gap: 8 }}>{tasks.map(task => <TaskRow key={task.task_id} task={task} t={t} onAction={act} canControl={pendingRequests.current.has(task.task_id)} />)}</div>}
  </div>;
}

function TaskRow({ task, t, onAction, canControl }: { task: AgentRuntimeTask; t: (key: any) => string; onAction: (task: AgentRuntimeTask, action: 'confirm' | 'pause' | 'cancel') => void; canControl: boolean }) {
