...
- 通过 `src/shared/layout/registry.ts` 的声明式 LayoutHost 排布 Ribbon、Sidebar 和主内容区；偏好「外观」中的布局预览器可立即切换已发现的布局。布局 mod 只能调整三个既有区域的方向、顺序、尺寸与 Sidebar 默认显隐，不能替换或执行区域组件。
